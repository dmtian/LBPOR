...
inline static void getBits(uint8_t * bits, uint64_t nl, uint16_t length) {
	uint64_t mask = nl;
...
struct Gaussian::GaussianSampling_sign::encapsulation {
	const double_t s;
	uint8_t modulus[k_sign];   // The base-ary decomposition sequence of module q
	double_t sigma; 
	Poly_s R[w][k_sign];  
	...
	inline void G_sampleD(int64_t * output, const double_t sigma, const double_t * c) const noexcept;
	void G_sampleG(int64_t * output, const uint64_t coset_i) const noexcept;
...
int64_t Gaussian::GaussianSampling_sign::encapsulation::GenerateIntegerKarney(const double_t stddev, const double_t mean) const noexcept{
		int64_t result;
...
void Gaussian::GaussianSampling_sign::encapsulation::GenerateVectorKarney(uint64_t *noise) const noexcept{
	...
	for(int32_t i=0;i<n;i++){
		...
		if(result<0)
		    noise[i]=uint64_t (q_sign+result);
	    else
...
inline void Gaussian::GaussianSampling_sign::encapsulation::G_sampleD(int64_t * output, const double_t sigmaa, const double_t * c) const noexcept {
	...
	double_t cs[k_sign];
	const int64_t zk_1 = output[k_sign - 1];
	for(uint32_t i = 0; i < k_sign; ++i) {
	    cs[i] =c[i]+ zk_1*G_d[i];
	}
    int64_t cs_iz;  
...
void Gaussian::GaussianSampling_sign::encapsulation::G_sampleG(int64_t * output, const uint64_t coset_i) const noexcept {
	// get bits of coset_i
	uint8_t u[k_sign];
...
void Gaussian::GaussianSampling_sign::encapsulation::P_samplePz(Poly_s *output) const noexcept {
	...
	for(uint32_t i = 0; i < n; ++i) {
		if(ifft_p[i].real()<0)
		    coefs_p[i] = q_sign - (uint64_t) (-ifft_p[i].real());
		else
			coefs_p[i] = (uint64_t) ifft_p[i].real();
...
void Gaussian::GaussianSampling_sign::encapsulation::P_sampleFz(std::complex<double_t> *output, const std::complex<double_t> *f, const std::complex<double_t> *c, const uint32_t length) const noexcept {
	if (length == 1) {
		//std::cout << "test67"<<fabs(f[0].real()) << std::endl;
...
Gaussian::GaussianSampling_sign::GaussianSampling_sign(const double_t s, Poly_s R[][k_sign]) noexcept : \
impl(new encapsulation {.s=s}) {
    for(uint32_t i = 0; i < w; ++i)
	    for(uint32_t j = 0; j<k_sign ; ++j)
	        impl->R[i][j]={R[i][j]};
...
*/
void Gaussian::GaussianSampling_sign::sampleGPoly(Poly_s * polys, const uint64_t * coset) const noexcept {
	uint64_t * samplingz = new uint64_t[n*k_sign];
	uint64_t * samplingzInd  = samplingz;
	int64_t samplingzIndex[k_sign];
	for(uint32_t i = 0; i < n; ++i) {
	...
	uint64_t * samplingzT = new uint64_t[n*k_sign];
	const uint64_t * samplingzInd2  = samplingz;
	uint64_t * samplingzTInd = samplingzT;
	for(uint32_t i = 0; i < k_sign; ++i) {
		for(uint32_t j = 0; j < n; ++j) {
			*samplingzTInd++ = (uint64_t) *(samplingzInd2 + k_sign*j);
		}
		++samplingzInd2;
	}
	delete[] samplingz;
	
	//Transforming the vector samplingzT of n*k_sign long into a polynomial vector of k_sign long.
